/*
  ...
  1. Changes
    - Improve handling of newline-separated IDs
    - Better validation of existing images
    - More robust URL construction
    - Prevent duplicate images
    
  2. Features
    - Support for both string and array inputs
    - Proper handling of all newline types (LF, CR, CRLF)
    - Validation of image existence in storage
    - Deduplication of image IDs
*/
  ...
  IF image_ids IS NOT NULL THEN
    ...
    LOOP
      ...
      IF EXISTS (
        ...
        -- Only add if image isn't already in the array
        IF NOT EXISTS (
          SELECT 1
          FROM jsonb_array_elements(NEW.images) AS img
          WHERE img->>'filename' = image_id
        ) THEN
          -- Construct the full URL and add the image
