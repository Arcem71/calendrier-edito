/*
  # Fix photo download trigger
  
  1. Changes
    - Update trigger function to handle newline-separated UUIDs
    - Construct storage URLs correctly using bucket name
    - Fix storage URL construction without relying on custom settings
    
  2. Features
    - Properly splits newline-separated UUIDs
    - Maintains existing images array structure
    - Uses correct Supabase storage URL format
*/
...
DECLARE
  ...
  storage_url TEXT;
BEGIN
  -- Get the current project URL from the supabase_url() function
